...
def add_new_apps(item, keyword):
	with DBLOCK:
		try:
			SQLins = "INSERT OR IGNORE INTO miniapps (docID, nickName, iconUrl, userName, description, path, scoreTfIdf, \
...
def add_searched_word(word):
	with DBLOCK:
		try:
			SQLins = "INSERT OR IGNORE INTO searched (wid, words) VALUES ((SELECT IFNULL(MAX(wid), 0) + 1 FROM searched), ?)"
